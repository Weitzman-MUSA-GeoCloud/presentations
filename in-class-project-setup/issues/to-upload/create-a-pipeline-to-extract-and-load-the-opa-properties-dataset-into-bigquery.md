...
1.  Fetch data and store in a folder in the `{{gcp_project}}-raw_data` bucket
2.  Convert data to a BigQuery-friendly external table format (e.g. JSON-L or Parquet) and store in a folder in the `{{gcp_project}}-prepared_data` bucket
3.  Create (or replace) a BigQuery external table in the `source` dataset based on the data in `gs://{{gcp_project}}-prepared_data`
